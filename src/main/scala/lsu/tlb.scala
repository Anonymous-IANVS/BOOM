...
class NBDTLB(instruction: Boolean, lgMaxSize: Int, cfg: TLBConfig)(implicit edge: TLEdgeOut, p: Parameters) extends BoomModule()(p) {
  ...
  val pmp = Seq.fill(memWidth) { Module(new PMPSMPUChecker(lgMaxSize)) }
  for (w <- 0 until memWidth) {
    pmp(w).io.addr := mpu_physaddr(w)
    pmp(w).io.size := io.req(w).bits.size
    pmp(w).io.pmp := (io.ptw.pmp: Seq[PMP])
    pmp(w).io.smpu := (io.ptw.smpu: Seq[SMPU])
  ...
  //val smpu= Seq.fill(memWidth) { Module(new SMPUChecker(lgMaxSize)) }
  //for (w <- 0 until memWidth) {
  //  smpu(w).io.addr := io.req(w).bits.vaddr
  //  smpu(w).io.size := io.req(w).bits.size
  //  smpu(w).io.smpu := (io.ptw.smpu: Seq[SMPU])
  //  smpu(w).io.prv := priv
  //}

  //val smpu_r   = widthMap(w => smpu(w).io.r)
  //val smpu_w   = widthMap(w => smpu(w).io.w)
  //val smpu_x   = widthMap(w => smpu(w).io.x)
  //val smpu_paddr   = widthMap(w => smpu(w).io.paddr)
  val is_smpu_working = widthMap(w => !vm_enabled(w) && (priv<PRV.S.U))

  //val legal_address = widthMap(w => edge.manager.findSafe(mpu_physaddr(w)).reduce(_||_))
  val legal_address = widthMap(w => edge.manager.findSafe(pmp(w).io.paddr).reduce(_||_))
  def fastCheck(member: TLManagerParameters => Boolean, w: Int) =
    legal_address(w) && edge.manager.fastProperty(pmp(w).io.paddr, member, (b:Boolean) => b.B)
    //legal_address(w) && edge.manager.fastProperty(mpu_physaddr(w), member, (b:Boolean) => b.B)
  //val cacheable = widthMap(w => (is_smpu_working(w) || fastCheck(_.supportsAcquireT, w) && (instruction || !usingDataScratchpad).B))
  ...
  //val prot_r   = widthMap(w => (is_smpu_working(w) || fastCheck(_.supportsGet, w) && pmp(w).io.r))
  //val prot_w   = widthMap(w => (is_smpu_working(w) || fastCheck(_.supportsPutFull, w) && pmp(w).io.w))
  //val prot_x   = widthMap(w => (is_smpu_working(w) || fastCheck(_.executable, w) && pmp(w).io.x))
  val cacheable = widthMap(w => fastCheck(_.supportsAcquireT, w) && (instruction || !usingDataScratchpad).B)
  val homogeneous = widthMap(w => TLBPageLookup(edge.manager.managers, xLen, p(CacheBlockBytes), BigInt(1) << pgIdxBits)(pmp(w).io.paddr).homogeneous)
  ...
  //val tlb_hit = widthMap(w => is_smpu_working(w)  || real_hits(w).orR)
  //val tlb_miss = widthMap(w => !is_smpu_working(w) && vm_enabled(w) && !bad_va(w) && !tlb_hit(w))
  ...
  for (w <- 0 until memWidth) {
    //when (io.req(w).valid && vm_enabled(w) && !is_smpu_working(w)) { //do not update sector/superpage hits when smpu is working
  ...
  for (w <- 0 until memWidth) {
    ...
    //io.resp(w).pf.ld   := !is_smpu_working(w) && ( (bad_va(w) && cmd_read(w)) || (pf_ld_array(w) & hits(w)).orR )
    //io.resp(w).pf.st   := !is_smpu_working(w) && ( (bad_va(w) && cmd_write_perms(w)) || (pf_st_array(w) & hits(w)).orR )
    //io.resp(w).pf.inst := !is_smpu_working(w) && ( bad_va(w) || (pf_inst_array(w) & hits(w)).orR)
    //io.resp(w).ae.ld   := !is_smpu_working(w) && ( (ae_ld_array(w) & hits(w)).orR)
    //io.resp(w).ae.st   := !is_smpu_working(w) && ( (ae_st_array(w) & hits(w)).orR)
    //io.resp(w).ae.inst := !is_smpu_working(w) && ( (~px_array(w)   & hits(w)).orR)
    //io.resp(w).ma.ld   := !is_smpu_working(w) && ( (ma_ld_array(w) & hits(w)).orR)
    //io.resp(w).ma.st   := !is_smpu_working(w) && ( (ma_st_array(w) & hits(w)).orR)
    ...
    //io.resp(w).cacheable    := !is_smpu_working(w) && (c_array(w) & hits(w)).orR //make SMPU data not-cachable
    //io.resp(w).must_alloc   := !is_smpu_working(w) && (must_alloc_array(w) & hits(w)).orR //make SMPU data not-must_alloc
    //io.resp(w).prefetchable := !is_smpu_working(w) && (prefetchable_array(w) & hits(w)).orR && edge.manager.managers.forall(m => !m.supportsAcquireB || m.supportsHint).B //make SMPU data not-prefetchable
    //io.resp(w).miss  := !is_smpu_working(w) && (do_refill || tlb_miss(w) || multipleHits(w)) //make SMPU data always hit
    ...
    io.resp(w).paddr := Mux(is_smpu_working(w), pmp(w).io.paddr, Cat(ppn(w), io.req(w).bits.vaddr(pgIdxBits-1, 0)))
  }
